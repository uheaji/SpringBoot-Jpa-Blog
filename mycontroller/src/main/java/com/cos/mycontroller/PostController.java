...
public class PostController {
	...
	// 데이터 전송(입력, insert) 목적이면 Post를 사용
	...
	public User testPostObject(User user) { // x-www-form-urlencoded
		return user; // MessageConverter가 낚아 챔 -> json으로 변환해서 리턴(RestController 일때만)
	}
	
	// @RequestBody를 사용하면 json 데이터를 받을 수 있음.
	@PostMapping("/test/post/json")
	public User testPostJson(@RequestBody User user) {  // json을 전송
		System.out.println(user); // MessageConverter가 낚아 챔 -> json으로 변환해서 리턴(RestController 일때만)
		return user;
	}
